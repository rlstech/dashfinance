...
import { useFilterTree, useFluxoObras, useImportarPlanilhaObras, useSavePlanejamento } from '@/hooks/useFinanceiro'
import { formatCurrency } from '@/lib/formatters'
import { Skeleton } from '@/components/ui/skeleton'
import { cn } from '@/lib/utils'
import type { FluxoMesRow, UpsertPlanejamentoIn } from '@/types'
...
export default function FluxoObras() {
  ...
  const [obraCodigo, setObraCodigo] = useState<string | null>(null)
  const [ano, setAno] = useState(currentYear)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const { data, isLoading } = useFluxoObras(obraCodigo, ano)
  const savePlanejamento = useSavePlanejamento()
  const importarPlanilha = useImportarPlanilhaObras()

  const allObras = tree ? Object.values(tree.obras_por_empresa).flat() : []
  const uniqueObras = [...new Set(allObras)].sort()

  const emptyMeses: FluxoMesRow[] = Array.from({ length: 12 }, (_, i) => ({
    mes: i + 1,
    custo_previsto: 0,
    receita_prevista: 0,
    custo_real: 0,
  ...
  const meses = data?.meses ?? emptyMeses

  // Fluxo acumulado calculado no frontend
  let accPlanned = 0
  let accReal = 0
  const acumuladoPlanejado = meses.map((m) => {
    accPlanned += m.receita_prevista - m.custo_previsto
    return accPlanned
  })
  ...
  function handleCellSave(
    mes: number,
    field: 'custo_previsto' | 'receita_prevista',
    value: number,
  ) {
    if (!obraCodigo) return
    const current = meses[mes - 1]
    const payload: UpsertPlanejamentoIn = {
    ...
    savePlanejamento.mutate(payload)
  }
  ...
  type RowDef =
    ...
    | { label: string; kind: 'readonly'; values: number[] }
    | { label: string; kind: 'accumulated'; values: number[] }
      ...
      <div className="p-6 md:p-8 overflow-auto flex-1">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* Seletor de obra */}
            <select
              className="text-xs font-bold uppercase tracking-widest border-2 border-dark px-3 py-2 bg-white focus:outline-none focus:border-brand max-w-[220px] truncate"
              value={obraCodigo ?? ''}
              onChange={(e) => setObraCodigo(e.target.value || null)}
            >
        ...
        {/* Estado: nenhuma obra selecionada */}
        {!obraCodigo ? (
          ...
          <>
            <div className="bg-white block-border shadow-hard overflow-x-auto">
              <table className="w-full text-xs border-collapse" style={{ minWidth: '900px' }}>
                <thead>
                  <tr className="bg-dark text-white">
                    <th className="text-left font-black uppercase tracking-widest px-4 py-3 sticky left-0 bg-dark z-10 w-52">
                      Métrica
                    </th>
                    {MESES.map((m) => (
                      <th
                        key={m}
                        className="text-right font-black uppercase tracking-widest px-2 py-3 w-20"
                      >
                        {m}
                      </th>
                    ))}
                ...
                <tbody>
                  {rowDefs.map((row, rowIdx) => {
                    ...
                    const rowBg = isAccumulated ? 'bg-bgBase' : rowIdx % 2 === 0 ? 'bg-white' : 'bg-white'
                    return (
                      <tr
                        key={row.label}
                        className={cn(
                          'border-b border-grid hover:bg-brand/5 transition-colors',
                          rowBg,
                        )}
                      >
                        <td
                          className={cn(
                            'px-4 py-2 font-bold uppercase tracking-wide text-xs sticky left-0 z-10 border-r-2 border-grid',
                        ...
                        {row.values.map((val, mesIdx) => (
                          <td key={mesIdx} className="px-1 py-1">
                            {row.kind === 'editable' ? (
                              <EditableCell
                                value={val}
                                disabled={!obraCodigo || savePlanejamento.isPending}
                                onSave={(v) => handleCellSave(mesIdx + 1, row.field, v)}
                              />
                            ) : (
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              ...
              Custo Real e Receita Realizada serão preenchidos automaticamente via UAU.
            </p>
          </>
        )}
      </div>
